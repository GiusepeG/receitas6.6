const ErrorHandlerService = {
  ...
  extractHeadlineOneFromText: function() {
    if (!this.apiKey) {
    ...
    const html = HtmlService.createHtmlOutputFromFile('client/dialogs/error_handler_client.html')
        .setWidth(600)
        .setHeight(480);
  ...
  processPastedText: function(pastedText, filterType) {
...
Agora, processe o texto a seguir e extraia o horário e o nome de cada paciente.`;
  ...
  writeToDocument: function(processedJsonString) {
    try {
      ...
      if (agendamentos && agendamentos.length > 0) {
        ...
        return `Documento atualizado com ${agendamentos.length} nomes.`;
      } else {
